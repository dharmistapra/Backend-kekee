...
const calculateShippingCost = async (weight, country) => {
    try {
        ...
        if (!shippingRules || shippingRules.length === 0) {
            return { success: false, message: "No shipping rules found for this country" };
        }
        ...
        return { success: false, message: "No matching shipping rule found for this weight" };
...
const findShippingPrice = async (req, res) => {
    try {
        ...
        if (!result.success) {
            return res.status(404).json({ isSuccess: false, message: result.message });
        }

        res.status(200).json({ isSuccess: true, data: { shippingCost: result.shippingCost } });
