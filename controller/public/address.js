...
const postshipAddress = async (req, res, next) => {
    try {
        const { email, fullName, country, state, city, zipCode, address1, address2, companyname, GstNumber, mobile, whatsapp, user_id, id } = req.body;
        ...
        if (id) {
            ...
            data = await prisma.billing.update({
                where: { id },
                data: {
                    fullName, email, city, country, state, zipCode, address1, address2, companyname, GstNumber, mobile, whatsapp, isSame: true,
                },
            });
            ...
            data = await prisma.billing.create({
                data: {
                    fullName, userId: user_id, email, city, country, state, zipCode, address1, address2, companyname, GstNumber, mobile, whatsapp, isSame: true,
                },
            });
