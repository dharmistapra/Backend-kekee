      ...
      await Promise.all(cartItems.map(async (item) => {
        ...
        if (isCatalogue && item.catalogue_id) {
          ...
          if (stitching) {
            ...
            item.Tax = priceDetails?.tax || 0;
            item.outOfStock = priceDetails.catalogueOutOfStock;
            stitchingData = await getAllStitchingData(parsedStitching, parsedStitching);
          }
        } else {
          if (size) {
            const priceDetails = await findproductpriceOnSize(
        ...
        return {
          ...
          category: item?.catalogue ? item?.catalogue.CatalogueCategory : item?.product?.categories,
          menu: menu,
          size: item.size,
          subtotal: item?.Subtotal,
          tax: item?.Tax,
