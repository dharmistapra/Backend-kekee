...
const userRegister = async (req, res, next) => {
  try {
    ...
    if (data)
      res
        .status(200)
        .json({ isSuccess: false, message: "Register successfully" });
  } catch (error) {
...
const SendOtp = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ isSuccess: false, message: "email does not existing" });
    }
  ...
  try {
    ...
    if (!otpVerified) {
      return res.status(400).json({
        isSuccess: false,
        message: "OTP verification failed. Cannot reset password.",
      });
    }
